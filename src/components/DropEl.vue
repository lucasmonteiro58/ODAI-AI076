...
export default {
  ...
  methods: {
    ...
    ondragenter(event) {
      ...
      const dropCenter = {
        x: dropRect.left - startPosX + this.dropzoneSnap.x + dropRect.width / 2,
        y: dropRect.top - startPosY + this.dropzoneSnap.y + dropRect.height / 2,
        range: Infinity
      }
    ...
    ondrop(event, dropped, snap) {
      if (this.replaceOndrop) {
        this.replaceOndrop(event, dropped, this.expected)
      } else {
        event.draggable.draggable({ enabled: false })
        const data = event.relatedTarget.getAttribute('data-transfer')
        const polaroidDropped = this.selectedObjects.filter(
          (p) => p.nome === data
        )[0]
        dropped.push(polaroidDropped)
        event.relatedTarget.offsetParent.offsetParent.style.transform =
          'rotate(0deg)'
        polaroidDropped.dropped = true
      }
    },
